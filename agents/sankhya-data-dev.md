...
2. **Parta do template oficial do Addon Studio** (quando disponível no ambiente): copie a estrutura e **esvazie o código de exemplo** (`model/src/.../exemplos/`, `datadictionary/tabela.xml`). Se não houver template local, reproduza o skeleton a partir de `references/addon-studio-estrutura.md` (autossuficiente). Siga os formatos/nomenclatura de lá.
