...
paths: "**/*.js, **/*.html, **/*.jsp, **/*.less"
---

# sankhya-js

Framework frontend proprietario baseado em **AngularJS 1.x** (nao Angular 2+). Publicado como `@sankhya-frontend/sankhya-js` ([package.json:2](../../../../sankhya-js/package.json#L2)), com `moduleName: "snk"` e `distName: "snk"`/`"jiva"` ([package.json:4-6](../../../../sankhya-js/package.json#L4-L6)). Build: **Grunt 0.4.5** + Babel + Less ([package.json:30-40](../../../../sankhya-js/package.json#L30-L40)).

## Regras nao-negociaveis

1. **AngularJS 1.x** (nao Angular moderno). Use `angular.module(...)`, DI por array, `$scope`, `$q`, `$http`. Nao use sintaxe de Angular 2+.
2. **Padrao de servico backend e `/{module}/service.sbr`** ([serviceproxy.service.js:150](../../../../sankhya-js/src/core/util/http/serviceproxy.service.js#L150)). Nunca monte a URL manualmente — use `ServiceProxy.callService` ou `ServiceProxy.builder()`.
3. **Prefixo de modulo no `serviceName`**: formato `"modulo@Servico.metodo"` (ex.: `"mgecom@FichaParceiroSP.getFinanceiros"`). Sem prefixo, `defModule = "mge"` ([serviceproxy.service.js:151-157](../../../../sankhya-js/src/core/util/http/serviceproxy.service.js#L151-L157)).
...
5. **Metadados de entidade com locale**: `MetadataProvider` cacheia por `entity + '-' + locale` ([metadataprovider.service.js:43-45](../../../../sankhya-js/src/core/metadataprovider/metadataprovider.service.js#L43-L45)). Trocar idioma invalida o cache implicitamente.
6. **Handles de `SkComponentRegistry` sao globais** — duplicidade gera `console.warn` e retorna `angular.noop` por default ([componentregistry.service.js:114-118](../../../../sankhya-js/src/core/services/registry/componentregistry.service.js#L114-L118)).
...
`core/` e infraestrutura do framework. `components/` sao componentes de UI — entre eles, o **`sk-dataset`** ([dataset.controller.js](../../../../sankhya-js/src/components/dataset/dataset.controller.js), ~7600 linhas) e a primitiva central de binding de dados (CRUD, listeners, metadados, navegacao). `commons/` contem telas (controller + service + html) que sao invocadas por mais de um modulo — por isso vivem fora dos modulos VC.
...
Builder em [serviceproxy.service.js:44-53](../../../../sankhya-js/src/core/util/http/serviceproxy.service.js#L44-L53); `callService` em [:122](../../../../sankhya-js/src/core/util/http/serviceproxy.service.js#L122).
...
Um `.module.js` por pasta declarando o modulo vazio ([button.module.js](../../../../sankhya-js/src/components/button/button.module.js)); demais arquivos (`*.directive.js`, `*.service.js`, `*.controller.js`) usam `angular.module('snk.components.meucomp')` **sem array de dependencias** ([button.directive.js](../../../../sankhya-js/src/components/button/button.directive.js)).
...
`get()` retorna promise que resolve assim que o `register()` correspondente for chamado ([componentregistry.service.js:80,107](../../../../sankhya-js/src/core/services/registry/componentregistry.service.js#L80)).
...
GET sincrono contra `/mge/metadataProvider.mge/{entity}.js` ([metadataprovider.service.js:103](../../../../sankhya-js/src/core/metadataprovider/metadataprovider.service.js#L103)). Deduplica requests em voo via `pendingEntityRequests`.
...
Permite telas carregadas **apos** o bootstrap declararem dependencias novas ([framework.config.js:54-73](../../../../sankhya-js/src/core/framework.config.js#L54-L73)).
