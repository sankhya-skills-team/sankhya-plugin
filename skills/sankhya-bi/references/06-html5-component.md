...
> **⚠️ ATENÇÃO — `JX.consultar` usa `DbExplorerSP` internamente:** Este serviço requer permissão de "Explorador de banco de dados" no perfil do usuário Sankhya. Na maioria dos ambientes de produção essa permissão **não está concedida** para usuários comuns, fazendo `JX.consultar` falhar silenciosamente ou retornar array vazio. **Para leitura de dados em produção, prefira carregar os dados via scriptlet Java no JSP** (conexão JDBC direta via JNDI) antes de servir o HTML — evitando dependência de permissões de runtime. Reserve `JX.consultar` apenas para ambientes onde `DbExplorerSP` é liberado (ex: usuários administradores ou ambientes de desenvolvimento).
...
### ❌ Assets por caminho relativo no JSP dão 404 — tela abre em branco

**Problema:** Um `html5component` (JSP servido via gadget) é carregado por uma URL do tipo `html5component.mge?entryPoint=dashboard/jsp/<x>.jsp&...&moduleContext=<addon>`. Assets referenciados por **caminho relativo** dentro do JSP resolvem para uma URL **errada no browser** (a base é `html5component.mge`, não o diretório do JSP) e dão **404**:

```html
<link href="../_tema/tema.css" rel="stylesheet">   <!-- 404 -->
<script src="../_tema/tema.js"></script>            <!-- 404 -->
```

Sintomas: a tela abre **em branco** (fundo branco, texto preto), o JS quebra com `ReferenceError: <Objeto> is not defined`, e o console mostra `tema.css 404` / `tema.js 404`.

**Solução robusta — embutir os assets via include estático server-side** do JSP. O `<%@ include %>` é resolvido no compile do JSP, relativo ao arquivo no webapp (imune ao caminho do browser), e mantém o arquivo de tema como fonte única:

```jsp
<style><%@ include file="../_tema/tema.css" %></style>
<script><%@ include file="../_tema/tema.js" %></script>
```

**Alternativa:** caminho absoluto do contexto do webapp (`/dashboard/_tema/...`), caso o include relativo não resolva.

> Ao inlinar, o arquivo incluído **não pode** conter `</script>`/`</style>` literais nem `<%`/`%>` — eles encerram o bloco ou são interpretados como código JSP.
...
private static final String FALLBACK_LOG_PATH = "/opt/wildfly/standalone/log/server.log";
