...
- `sankhya-doc-entrega-html`: consultar apenas como referência de linguagem e estrutura documental quando o usuário pedir documento de entrega; não confundir escopo com entrega concluída.
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/sankhya-gera-escopo/scripts/create_scope_docx.py" \
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/skills/sankhya-gera-escopo/scripts/docx_to_markdown_fallback.py" \
  /caminho/Escopo.docx \
