...
- `NativeSql.releaseResources(q)` — equivalente estático null-safe a `q.close()`. Utilizado em `InMemoryDataSet.java:150-151` (JAPE) e em Centrais do MGECom.
...
Padrão usado nas Centrais (`CACHelper.java:1501-1506`, MGECom-Model):
...
`setEntity` aparece logo após `createEntity` no fluxo de INSERT; a proximidade induz à confusão de que ele participa da persistência. A implementação (`PrePersistEntityState.java:142`, JAPE) atribui o campo `this.entity`.
