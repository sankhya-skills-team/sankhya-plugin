...
├── SKILL.md                            Instruções principais e método obrigatório
└── references/
    ├── heuristicas-erp.md              Classificação de trechos, domínios, heurísticas ERP,
    │                                   priorização, conduta ante ambiguidade,
    │                                   tabela de decisão Módulo Java × Addon Studio
    └── heuristicas-estimativa.md       Base por artefato, multiplicadores automáticos,
                                        buffer de 30%, nível de confiança,
                                        fluxo de refinamento por esqueleto de código
```
...
```bash
git clone https://github.com/ratk/skill-sankhya-estimativa-planejador \
    ~/.claude/skills/sankhya-estimativa-planejador
```
