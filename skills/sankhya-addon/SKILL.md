...
> **Acesso a dados — JAPE-first + API verificada (REGRA DURA):** preferir **JAPE**
> (`JapeFactory.dao(...)`, `EntityFacade`/`DynamicVO`, `FinderWrapper`); `NativeSql`
> só quando JAPE não expressa a consulta, com **parâmetro nomeado**
> (`appendSql`+`setNamedParameter`+`executeQuery`). **PROIBIDO inventar assinatura
> de API** — verificar no jar antes de usar método de `NativeSql`/util/helper.
> Métodos inexistentes que já causaram bug: `NativeSql.getColumnsAsMap(...)`,
> `MapUtils.getMapValueAsBigDecimal(...)`. Detalhes e CRUD canônico em
> `references/jape.md`; interceptar buscas em `references/before-load-listener.md`.
...
| **CRUD Service Listener** | Interceptar CRUD no dataset de uma tela específica (campos transientes da grade); mecanismo legado `DatasetCRUDListenerAdapter` do modelcore | `references/crud-service-listener.md` |
...
| **SQL Multibanco** | Escrever SQL portável Oracle ↔ SQL Server: tipos, funções proibidas, nomenclatura, idempotência de DML | `references/sql-multibanco.md` |
