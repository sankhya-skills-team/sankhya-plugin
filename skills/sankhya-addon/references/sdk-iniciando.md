...
## buildscript completo v2 (validado em build real, projeto real, 2026)

O `gradle-plugin:2+` processa as anotações via **KSP** — sem o KSP e o Kotlin
plugin no classpath, `br.com.sankhya.studio.annotations` não fica disponível e
`compileJava` falha com *"package ... does not exist"*. Bloco mínimo que compila:
...
plugins { id 'co.uzzu.dotenv.gradle' version '4.0.0' }   // le WILDFLY_HOME do .env
...
```
Pipeline do plugin 2: `convertMetadata → kspKotlin → compileJava`.
