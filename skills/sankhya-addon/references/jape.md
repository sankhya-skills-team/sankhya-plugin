...
Métodos básicos: `appendSql(String)`, `setNamedParameter(String, Object)`,
`executeQuery()` (retorna `ResultSet`). Para leitura tabular há também
`NativeSql.getColumnsAsMap(...)` / `getResultSetRowAsMap(...)` e os helpers
`MapUtils.getMapValueAs*` — assinaturas reais e exemplos em `nativesql-assertivo.md`.
SQL nativo exige parâmetro nomeado (anti-injection); nunca concatenar entrada do usuário.
