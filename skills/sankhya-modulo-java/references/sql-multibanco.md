...
Regras para escrever SQL que roda **igual** em Oracle e SQL Server (Sankhya suporta ambos). Aplica a scripts de banco do módulo, queries de NativeSql/`.sql`, views e qualquer SQL embutido no código.
...
Sempre use **prefixo único** (do projeto/cliente) em todas as tabelas, colunas e objetos — evita colisão com tabelas nativas do Sankhya e de outros módulos. Não use o prefixo genérico `AD_`.
