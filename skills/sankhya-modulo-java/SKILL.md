...
> **Acesso a dados — JAPE preferencial + API verificada (REGRA DURA):**
> Preferir **JAPE** (`JapeFactory`/`EntityFacade`/`FinderWrapper`). `NativeSql` é
> legítimo no `repository/` **apenas** para não disparar eventos de persistência —
> não como atalho. **PROIBIDO inventar assinatura de API.** Antes de chamar
> qualquer método de `NativeSql`, `DwfUtils`, `MapUtils` ou helper, verificar a
> assinatura real no jar (`javap`/grep no classpath/`references/`). Métodos
> inexistentes já causaram bug: `NativeSql.getColumnsAsMap(...)`,
> `MapUtils.getMapValueAsBigDecimal(...)`. Sem confirmar a assinatura, usar a via
> JAPE equivalente. API correta do `NativeSql`: `appendSql` + `setNamedParameter`
> + `executeQuery`. Detalhes em `references/jape.md`.
