...
Variante observada em `AgendadorRelatoriosPopUp.js:71-78`: `setTimeout` envolvendo `$popupInstance.dismiss` dentro de um `.catch` para "mover o throw para fora da cadeia de promise".
...
- `isLoaded()` fica `true` **para sempre** apos o primeiro load (`dataset.controller.js:1119`). Em refreshes subsequentes continua `true`, entao polling nao deteta "terminou este refresh".
...
Todos os `add*Listener` do dataset retornam **funcao de desregistro** (ver todos os `add*Listener` em `dataset.controller.js:5904-6188`). Se a tela e reaberta sem recarregar o bundle, o listener antigo continua. Na segunda sessao, cada save dispara o listener 2x. Terceira, 3x. E assim por diante.

Vale o mesmo para `ServiceProxy.addClientEvent` — retorna `ClientEventHandler` com `.unregistry()` (`serviceproxy.service.js:86`).
...
Nao aplica: headers `appkey`/`sktk`, counter incremental, `mgeSession`, `resourceID`, `globalID`, `flowID`, popup de erro padrao, client events, print listeners, monitoramento de rede, modulo em update, serializacao de chamadas, suporte a abort. Tudo isso esta em `serviceproxy.service.js:150-394`.
...
Sem `@`, cai em `defModule = "mge"` (`serviceproxy.service.js:151`). Request vai para `/mge/service.sbr?...` e o backend retorna erro de "servico nao encontrado".
...
Se `config.callback` esta setado, o `.then` nunca recebe o dado (`serviceproxy.service.js:289-293`). Handler B so dispara em caso de erro (e mesmo assim so se `exceptionCallback` nao estiver setado).
...
O segundo `register` com mesmo handle gera `console.warn` e retorna `angular.noop` (`componentregistry.service.js:114-118`). A instancia antiga continua ativa — se ja foi destruida, `get(handle)` resolve com zumbi.
...
`callService` retorna promise imediatamente. O erro vai via rejection, nao exception. O `try/catch` so pegaria erro na **construcao** da chamada (ex.: metodo HTTP invalido — `serviceproxy.service.js:134`).
...
O ServiceProxy **ja exibe popup de erro** por default em `handleDefaultSystemError` (`serviceproxy.service.js:447-459`). O codigo acima abre o popup do framework **e** o popup manual — usuario ve dois modais sobrepostos.
...
O `$get` do provider lanca `Error('Este injector deve ser usado apenas em configurações de módulos.')` (`framework.config.js:47`).
...
Usar `SkWorkspace` — todos os metodos fazem `workspace && workspace.xxx()` (`workspace.service.js:124,131,...`).
