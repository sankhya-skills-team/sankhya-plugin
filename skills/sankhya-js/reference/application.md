...
Atributos reconhecidos no `compile` (`directive:28-56`):
...
Provider com tres modos (`skapplication.service.js:3-84`):
...
Mixin de `EventBus` aplicado no `self` (`controller:29`): `dispatchEvent`, `addEventListener`, etc.
...
Registrados se `!has-embedded-swf` (`controller:634-761`). Todos disparados como eventos `KeyboardEvents.*` no EventBus do controller — componentes escutam via `app.addEventListener(KeyboardEvents.SAVE_KEY, ...)`.
...
Ocorrem no construtor do controller, antes de `$scope.loadByPK` ser chamado (`controller:86-243`):
