...
Para consultas em tempo real no banco, usar o MCP `sankhya-schema` (`run_query`/`describe_table`).
...
## Consulta ao Banco em Tempo Real

Para tabelas não cobertas pelos arquivos de referência ou para dados frescos, usar o
MCP `sankhya-schema` — ele consulta o schema real da instância conectada:

- `describe_table(TABELA)` — campos, tipos, tamanhos
- `search_columns(...)` / `search_tables(...)` — localizar campo/tabela
- `get_foreign_keys(TABELA)` — FKs reais
- `run_query(SQL)` — consulta livre (ex.: `TDDCAM`, `TDDOPC`, `TDDLIG`)

As tabelas de referência desta skill são um retrato pré-extraído; para o estado atual
do banco de um cliente específico, o MCP é a fonte autoritativa.
...
- **`sankhya-doc-entrega-html`** — nomes corretos de campos persistidos ao documentar entregas
