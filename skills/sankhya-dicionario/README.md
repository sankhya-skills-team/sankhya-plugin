...
**Data da extração:** 2026-04-14 (retrato de um banco Sankhya OM de referência).
Para o estado atual do banco de um cliente específico, consultar o MCP `sankhya-schema`.
...
## Dados frescos / tabelas não cobertas

As tabelas de referência são um retrato pré-extraído. Para o estado atual do banco de
um cliente ou tabelas fora da cobertura, consultar o MCP `sankhya-schema`
(`describe_table`, `search_columns`, `get_foreign_keys`, `run_query`) — fonte
autoritativa do schema da instância conectada.
