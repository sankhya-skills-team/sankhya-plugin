...
A diretiva [sk-datagrid](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L146-L314) seleciona o controller em tempo de `compile` via atributo:
...
Evidencia: [datagrid.directive.js:221-244](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L221-L244). O motor canonico e AG-Grid ([datagrid.aggrid.controller.js](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js), 7352 linhas). `uigrid`/`jqx` sao paths legados mantidos.
...
No linker, `DatasetUtils.buildDatasetStandaloneAtObject(obj, scope)` cria o dataset e chama `initAndRefresh()` ([datagrid.directive.js:279-282](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L279-L282)). Uso real: [Indicadores.tpl.html:55-59](../../../../sankhya-js/src/commons/fichaparceiro/Indicadores/Indicadores.tpl.html#L55).
...
|-------|-----------|-------------|
| `attrs.id="x"` — publica no `$scope.$parent[x]` | [datagrid.directive.js:296-298](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L296-L298) | Acessar direto do mesmo controller (ex.: `$scope.gridContrato.refreshView()`) |
| `sk-on-datagrid-loaded="fn(datagrid)"` | [:167](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L167) | Precisa esperar o grid montar antes de tocar a API |
...
Todos opcionais exceto `sk-dataset` (ou `sk-standalone-dataset-object`). Lista completa em [datagrid.directive.js:158-219](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L158-L219).
...
| Atributo | Default | Uso |
|----------|---------|-----|
| `sk-disable-pagination` | `false` | **Atencao: pode causar problemas de desempenho** ([diretiva, linha 22](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L22)) |
...
`sk-drag-enabled`, `sk-drop-enabled`, `sk-drag-field-presentation`, `sk-drag-start`, `sk-drag-enter`, `sk-drag-over`, `sk-drop`, `sk-drag-end`, `sk-drop-tree` — todos em [:180-188](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L180-L188).
...
Evidencia: [:190-191](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L190-L191).

### Eventos de linha / celula

`sk-on-dbl-click`, `sk-on-click`, `sk-click-row-focus`, `sk-fn-save-row-on-enter`, `sk-fn-save-row-on-arrow-down`, `sk-fn-save-on-click`, `sk-new-row-on-tab`, `sk-new-row-on-arrow-down`, `sk-enter-as-tab`, `sk-arrow-focus-field`, `sk-nextrow-aftersave`, `sk-focus-out` — ver [:164-216](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L164-L216).

---

## API do controller (`SkAgGridController`)

Todos os metodos listados estao em [datagrid.aggrid.controller.js:521-601](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L521-L601).
...
Evidencia: [:6343-6348](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L6343-L6348). **Atencao: nao retorna funcao de desregistro** — a funcao fica na fila ate a proxima inicializacao. Evitar em fluxos que reabrem a tela.

---

## Integracao com dataset (observer)

O grid se auto-registra como `DatasetObserver` no dataset vinculado, via o contrato `allEvents`/`refreshed`/`currentLineChanged`/etc. documentado em [dataset.md](dataset.md). Handlers automaticos em [:2751-2759](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L2751-L2759):
...
Diretiva filha para sobrescrever ou acrescentar propriedades de coluna. Pode existir dentro de `sk-datagrid` ou `sk-dynaform`. Contrato em [datagridcustomcolumn.directive.js:22-149](../../../../sankhya-js/src/components/datagrid/datagridcustomcolumn.directive.js#L22-L149).
...
Exemplo real: [Indicadores.tpl.html:91-113](../../../../sankhya-js/src/commons/fichaparceiro/Indicadores/Indicadores.tpl.html#L91-L113).
...
Filhos do `sk-datagrid` para injetar renderizacao/editor HTML5 custom por coluna. Processados via `eval` em [datagrid.directive.js:246-275](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L246-L275) — a tag `<script columns="NOME_COL">` deve conter `function renderer() { return { getGui, refresh, destroy } }`.
...
Exemplo no docblock da diretiva: [datagrid.directive.js:55-108](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L55-L108).
...
A funcao e empilhada em `_gridReadyListeners` e nunca removida por API publica ([:6343-6348](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L6343-L6348)). Em fluxos que montam/desmontam o grid varias vezes (ex.: tabs lazy, popups reabertos), preferir `sk-on-datagrid-loaded`.

### 2. Selecao multipla desligada por default

`sk-allow-multiple-selection` nao informado = so selecao unica. Checado em [:5427-5429](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L5427-L5429).

### 3. `sk-disable-pagination` tem custo

A propria diretiva alerta: `"Atenção: desabilitar a paginação pode causar sérios problemas de desempenho."` ([diretiva, linha 22](../../../../sankhya-js/src/components/datagrid/datagrid.directive.js#L22)).
...
O controller e tambem acessivel via `$element.data('$skDatagridController')` ([:4](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L4)) — util para codigo de baixo nivel que tem o elemento DOM mas nao o escopo.
...
Nao sobrescrever `grid.refreshed`, `grid.dataSaved`, etc. — esses slots sao usados pelo proprio controller para reagir a eventos do dataset ([:2751-2759](../../../../sankhya-js/src/components/datagrid/datagrid.aggrid.controller.js#L2751-L2759)).
