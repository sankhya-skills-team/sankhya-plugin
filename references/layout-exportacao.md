...
- `markup="html"` — **atributo de `<textElement>`, não de `<textField>`** — interpreta HTML básico (`<b>`, `<i>`, `<br/>`)

**Erro 4.0.1:** colocar `markup=` em `<textField>` causa `SAXParseException`: "atributo 'markup' não pode aparecer no elemento 'textField'"
